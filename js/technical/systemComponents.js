var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<span v-if="player.points.lt('1e1000')"  class="overlayThing" >Your ordinal is </span>
		<h2  class="overlayThing" id="points"><span v-html="ordinal"></span></h2>
    ...
    'info-tab': {
        ...
        <span v-if="modInfo.author">
            ...
            Inspired by <a class="link" href="https://flamemasternxf.github.io/ordinal-pringles/" v-bind:style="{'font-size': '14px', 'display': 'inline'}"">Ordinal Pringles</a> by Flamemaster
        </span>
        <br>
    ...
    'options-tab': {
        ...
        <tr>
                <td><button class="opt" onclick="toggleOpt('gwaOrdinal')">gwaify ordinal: {{ options.gwaOrdinal?"ON":"OFF" }}</button></td>
                <td><button class="opt" onclick="changeMaxOrdinalLength()">Change max ordinal length</button></td>
                <td><button class="opt" onclick="toggleOpt('bhoExponent')">BHO display: {{ options.bhoExponent?"Exponential":"Multiplicative" }}</button></td>
				</tr> 
        <tr>
                <td><button class="opt" onclick="importOther()">Import below save</button></td>
				</tr> 
        <tr><td><input type="text" id="saveImport" size=11></td></tr>
        </table><br>
